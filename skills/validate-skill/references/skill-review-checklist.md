...
Load this file during the validate-skill workflow to perform a self-review before finalizing.
...
| A-7 | ⚠️ | `disable-model-invocation: true` is set if the skill creates files, runs commands, or has other side effects | Check if Write/Bash/Edit is in `allowed-tools` → if yes, verify this flag |
...
| I-6 | ⚠️ | Skill name follows verb+purpose pattern (e.g., `create-skill`, `reflect`, `validate-skill`) | Check `name:` in frontmatter |
